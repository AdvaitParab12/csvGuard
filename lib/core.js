...
export default function validateAndClean(records, headers) {
  ...
  for (let record of records) {
    ...
    if (!v.isLinkedInURL(record[headers.linkedin])) {
      recordError.push("LinkedIn url is not valid");
    }

    // Validate employee size
    if (!v.isEmployeeSize(record[headers.employeeSize])) {
      recordError.push("Employee size is not valid");
    }

    // Validate website url
    if (!v.isURL(record[headers.website])) {
      recordError.push("Website URL is not valid");
    }

    if (!v.isEmail(record[headers.email])) {
      recordError.push("Email is not valid");
    }

    if (!v.isLocation(record[headers.location])) {
